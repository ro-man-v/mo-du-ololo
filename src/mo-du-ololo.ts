...
export class Moduololo {
   ...
   */
  public moduloN(binaryString: string, n: number): number {
   ...
   */
  private calculateNewRemainder(remainder: number, binaryDigit: number, n: number): number {
    const newRemainder = remainder * this.base + binaryDigit;
    const division = Math.floor(newRemainder / n);

    return newRemainder - division * n;
  }
}
